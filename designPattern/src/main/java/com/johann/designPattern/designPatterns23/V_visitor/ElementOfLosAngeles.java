 ...
 * @see
 **/
public class ElementOfLosAngeles extends Element {
    /**
     * 双分派
     * 首先在客户端程序中,将具体访问者作为参数传递到ElementOfLosAngeles中(第一次分派,多态实现动态分派)
     * 然后,ElementOfLosAngeles类调用作为参数的"具体访问者"的visit()方法,同时将自己(this)作为参数传递进去。(第二次分派,静态分派)
     *
